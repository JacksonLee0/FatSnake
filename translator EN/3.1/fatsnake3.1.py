...
def getResultGoogle(target: 'str', sl, tl):
    allText = ['', '', '', '']
    if target == '':
        return ''
    else:
...
class Window(wx.Frame):
    def __init__(self, parent, title):
        ...
        self.fontName = 'DengXian'
        self.font4label = wx.Font(13, wx.DEFAULT, wx.NORMAL, wx.NORMAL, faceName=self.fontName)
        self.font4trans = wx.Font(11, wx.DEFAULT, wx.NORMAL, wx.NORMAL, faceName=self.fontName)

        self.font4btn = wx.Font(10, wx.DEFAULT, wx.NORMAL, wx.NORMAL, faceName=self.fontName)
        self.font4tfoot = wx.Font(8, wx.DEFAULT, wx.NORMAL, wx.NORMAL, faceName=self.fontName)
...
class DialogAccount(wx.Dialog):
    def __init__(self, parent, title, accountInformation):
        ...
        self.LogOutButton = wx.Button(self.panelDialogAccount, label='退出登录', pos=(60, 120), size=(60, 25))
        self.ResetPasswordButton = wx.Button(self.panelDialogAccount, label='修改密码', pos=(160, 120), size=(60, 25))
        self.Bind(wx.EVT_BUTTON, self.ResetPassword, self.ResetPasswordButton)
        self.Bind(wx.EVT_BUTTON, self.checkLogOut, self.LogOutButton)

        if accountInformation[0] is False:
            self.userName.Hide()
            self.LogOutButton.Hide()
            self.ResetPasswordButton.Hide()
    ...
    def ResetPassword(self, event):
        self.changePasswordDialog = DialogChangePassword(self, '修改密码窗口', self.accountInformation[1])
        self.changePasswordDialog.ShowModal()


class DialogChangePassword(wx.Dialog):
    def __init__(self, parent, title, userName):
        super(DialogChangePassword, self).__init__(parent, title=title, size=(300, 200))
        self.userName = userName
        self.panelDialogChangePassword = wx.Panel(self)

        self.oldPasswordLabel = wx.StaticText(self.panelDialogChangePassword, pos=(30, 30), label='旧密码：')
        self.newPasswordLabel = wx.StaticText(self.panelDialogChangePassword, pos=(30, 60), label='新密码：')
        self.passwordConfirmationLabel = wx.StaticText(self.panelDialogChangePassword, pos=(30, 90), label='密码确认：')

        self.oldPasswordInput = wx.TextCtrl(self.panelDialogChangePassword, pos=(100, 30), size=(150, 22))
        self.newPasswordInput = wx.TextCtrl(self.panelDialogChangePassword, pos=(100, 60), size=(150, 22),
                                            style=wx.TE_PASSWORD)
        self.passwordConfirmationInput = wx.TextCtrl(self.panelDialogChangePassword, pos=(100, 90), size=(150, 22),
                                                     style=wx.TE_PASSWORD)

        self.confirmButton = wx.Button(self.panelDialogChangePassword, label='确定', pos=(110, 120), size=(60, 25))
        self.Bind(wx.EVT_BUTTON, self.checkPassword, self.confirmButton)
        self.Center()

    def checkPassword(self, event):
        if self.newPasswordInput.GetValue() == '' or self.newPasswordInput.GetValue() == '' or self.passwordConfirmationInput.GetValue() == '':
            wx.MessageBox('密码项不能为空！', '提示！', wx.OK | wx.ICON_INFORMATION)
        elif userInfo.find_one({'_id': self.userName})['password'] != self.oldPasswordInput.GetValue():
            wx.MessageBox('旧密码输入不正确！\n请重新输入！', '提示！', wx.OK | wx.ICON_INFORMATION)
        elif not self.newPasswordInput.GetValue().isalnum() or len(self.newPasswordInput.GetValue()) < 8:
            wx.MessageBox('新密码格式不正确！\n密码应包含字母和数字，长度至少为8位！\n请输入正确格式的密码！', '提示！', wx.OK | wx.ICON_INFORMATION)
        elif self.newPasswordInput.GetValue() != self.passwordConfirmationInput.GetValue():
            wx.MessageBox('密码确认不一致！请重新输入！', '提示！', wx.OK | wx.ICON_INFORMATION)
        else:
            wx.MessageBox('修改密码成功！', '提示！', wx.OK | wx.ICON_INFORMATION)
            self.doResetPassword(self)
            self.Close()

    def doResetPassword(self, event):
        condition = {'_id': self.userName}
        user = userInfo.find_one(condition)
        user['password'] = self.newPasswordInput.GetValue()
        userInfo.update_one(condition, {'$set': user})


if __name__ == '__main__':
    url = "mongodb://dongming-asia-2" \
          ":jDMRV6fwHXcVNNBw7bLonz7e9OXgPPeFHOOc7EbCAdEJJvOj4nIxW9FKaV2Bb1xc9hFl0dc8eOP6TsikGvFwxQ==@dongming-asia-2" \
